...
def test_download():
    imgs = LandsatDownloader.download_scene(
        bands=['BQA'], scene_id=SCENE_ID_00)
    assert(len(imgs) == 2)
    assert('BQA' in [i['type'] for i in imgs])
    assert('MTL' in [i['type'] for i in imgs])
    imgs = LandsatDownloader.download_scene(
        bands=['BQA'], scene_id=SCENE_ID_00, metadata=False)
    assert(len(imgs) == 1)
    assert('BQA' in [i['type'] for i in imgs])
...
def test_with_wrong_scene_id():
    imgs = LandsatDownloader.download_scene(
        bands=['BQA'], scene_id=SCENE_ID_01)
    assert(len(imgs) == 2)
    assert('BQA' in [i['type'] for i in imgs])
    assert('MTL' in [i['type'] for i in imgs])

    imgs = LandsatDownloader.download_scene(
        bands=['BQA'], scene_id=SCENE_ID_01, metadata=False)
    assert(len(imgs) == 1)
    assert('BQA' in [i['type'] for i in imgs])
