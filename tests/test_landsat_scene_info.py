...
from landsat_downloader.scene_info import SceneInfo, Info
...
def test_scene_info_product_rt():
    ...
    scene_info = SceneInfo(scene_id=scene_id)
...
def test_scene_info_product():
    ...
    assert(scene_info.info.path == 224)
    assert(scene_info.info.row == 68)
    assert(scene_info.info.year == 2018)
    assert(scene_info.info.gsi == "LGN")
    assert(scene_info.info.version == "00")
    assert(scene_info.info.acq_date == datetime(2018, 3, 10).date())
...
def test_scene_info_julian_2_date():
    scene = "LC82240682018069LGN00"
    scene_info = Info(scene_id=scene)
...
def test_scene_info_product_loop():
    ...
    for i in range(2, 5):
        s = "{}{}".format(scene, i)
        scene_info = SceneInfo(scene_id=s, update_scene=True)
